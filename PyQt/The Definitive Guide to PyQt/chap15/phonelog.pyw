...
def createFakeData():
    ...
    while query.next():
        ...
        caller = str(query.value(1).toString())
        starttime = str(query.value(2).toDateTime().toString(
                            DATETIME_FORMAT))
        endtime = str(query.value(3).toDateTime().toString(
                          DATETIME_FORMAT))
        topic = str(query.value(4).toString())
