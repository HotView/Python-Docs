...
def createFakeData():
    ...
    while query.next():
        ...
        caller = str(query.value(CALLER).toString())
        starttime = (str(query.value(STARTTIME).toDateTime()
                     .toString(DATETIME_FORMAT)))
        endtime = (str(query.value(ENDTIME).toDateTime()
                   .toString(DATETIME_FORMAT)))
        topic = str(query.value(TOPIC).toString())
        ...
        if subquery.next():
            outcome = str(subquery.value(0).toString())
